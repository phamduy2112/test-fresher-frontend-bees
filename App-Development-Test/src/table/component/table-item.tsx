...
interface TableItemProps {
  ...
  const TableItem: React.FC<TableItemProps> = ({ index, user }) => {
  ...
  <tr key={index} className={index % 2 ==0?'bg-gray-100 text-center':'text-center'}>
            ...
            />
              {user.name}</td>
            <td className="border border-gray-300 p-2">${user.balance.toLocaleString()}</td>
            <td className="border border-gray-300 p-2">
            <a href={`mailto:${user.email}`} className="text-blue-500">{user.email}</a>

            </td>
            <td className="border border-gray-300 p-2" title={user.registerAt.toString()}>
              {user.registerAt.toLocaleDateString('en-CA')}
            </td>
            <td className="border border-gray-300 p-2">
